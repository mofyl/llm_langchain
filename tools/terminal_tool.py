...
class TerminalTool(Tool):
    ...
    def __init__(
        ...
        super().__init__(
            name="terminal",
            desc="跨平台命令行工具 - 执行安全的文件系统、文本处理和代码执行命令（支持Windows/Linux/Mac）",
        )
    ...
    def run(self, param: dict[str, Any]) -> str:
        ...
        if not self.validate_parameters(parameters=param):
            return "❌ 参数验证失败"
        command = param.get("command", "").strip()
        ...
        if base_command == "cd":
            return self.handler_cd(parts=parts)

        return self._execut_command(command=command)

    def handler_cd(self, parts: list[str]) -> str:
        """处理 cd 命令"""
        if self.allow_cd:
            return "❌ cd 命令已禁用"
        if len(parts) < 2:
        ...
        # 检查是否在工作目录内
        try:
            # relative_to 这个方法会去检查 路径必须存在包含关系
            # new_dir 必须在  self.workspace 下
    ...
    def _execut_command(self, command: str) -> str:
        """执行命令"""
