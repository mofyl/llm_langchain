...
from sympy import content, limit, true
...
class NoteTool(Tool):
    ...
    def __init__(
        self, workspace: str = "./notes", auto_backup: bool = True, max_notes: int = 1000, expandable: bool = False
    ):
        super().__init__(
            name="note",
            desc="笔记工具 - 创建、读取、更新、删除结构化笔记，支持任务状态、结论、阻塞项等类型",
    ...
    def _load_index(self):
        ...
        if self.index_file.exists():
            with open(self.index_file, encoding="utf-8") as fd:
                self.notes_index = json.load(fd)
        else:
            self.notes_index = {
                "notes": [],
                "metadata": {
    ...
    def _save_index(self):
        with open(self.index_file, "w", encoding="utf-8") as fd:
            json.dump(self.notes_index, fd, ensure_ascii=False, indent=2)
    ...
    def _create_note(self, title: str, content: str, note_type: str = "general", tags: list[str] | None = None) -> str:
        """创建笔记
        ...
        note_path = self._get_note_path(note_id=note_id)
        markdown_content = self._note_to_markdown(note=note)
    ...
    def _read_note(self, note_id: str) -> str:
        ...
        with open(note_path, encoding="utf-8") as fd:
            markdown_text = fd.read()

        note = self._markdown_to_note(markdown_text)

        return self._format_note(note=note)
    ...
    def _update_note(
        ...
        note = self._markdown_to_note(markdown_text=markdown_text)
        ...
        if tag is not None:
            note["tags"] = tag if isinstance(tag, list) else []
    ...
    def _delete_note(self, note_id: str) -> str:
        ...
        self.notes_index["metadata"]["total_notes"] = len(self.notes_index)
        self._save_index()

        return f"✅ 笔记已删除: {note_id}"

    def list_notes(self, note_type: str | None = None, limit: int = 10) -> list[dict[str, Any]]:
        """列出笔记
        ...
        filtered_notes = self.notes_index["nodes"]
    ...
    def _list_notes(self, note_type: str | None = None, limt: int = 10) -> str:
        """列出笔记
    ...
    def _search_note(self, query: str, limit: int = 10) -> str:
        """搜索笔记
    ...
    def search_note(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        """搜索笔记
    ...
    def get_parameters(self) -> list[ToolParameter]:
        ...
        return [
            ...
            ToolParameter(name="title", type="string", description="笔记标题（create/update时必需）", required=False),
            ToolParameter(name="content", type="string", description="笔记内容（create/update时必需）", required=False),
            ToolParameter(
                name="note_type",
                type="string",
                description=(
            ...
            ToolParameter(name="tags", type="array", description="标签列表（可选）", required=False),
            ToolParameter(
                name="note_id", type="string", description="笔记ID（read/update/delete时必需）", required=False
            ),
            ToolParameter(name="query", type="string", description="搜索关键词（search时必需）", required=False),
            ToolParameter(
                name="limit", type="integer", description="返回结果数量限制（默认10）", required=False, default=10
            ),
        ]

    def run(self, param: dict[str, Any]) -> str:
        """执行工具（非展开模式）"""
        if not self.validate_parameters(param):
            return "❌ 参数验证失败"
        ...
        if action == "create":
            self._create_note(
                ...
                note_type=param.get("note_type", "general"),
                tags=param.get("tags"),
            )
        elif action == "read":
            self._read_note(note_id=param.get("note_id"))
        elif action == "update":
            ...
            return self._delete_note(note_id=param.get("note_id"))
        elif action == "list":
            return self._list_notes(note_type=param.get("note_type"), limit=param.get("limit", 10))
        elif action == "search":
            return self._search_note(query=param.get("query"), limit=param.get("limit", 10))
        elif action == "summary":
