...
class SimpleAgent(Agent):
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        if not self.enable_tool_calling:
            ...
            self.add_message(message=Message(content=resp, role=RoleType.ASSISTANT))
            return resp
            # 迭代处理，支持多轮工具调用
        current_iteration = 0
        final_response = ""
        ...
        if final_response is None:
            return "未找到"
        self.add_message(Message(content=input_text, role=RoleType.USER))
        self.add_message(Message(content=final_response, role=RoleType.ASSISTANT))
